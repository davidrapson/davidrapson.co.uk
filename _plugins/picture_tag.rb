    ...
    def render(context)
      ...
      if settings['markup'] == 'picture'
        ...
        source_keys.each { |source|
          ...
          source_tags += "<source srcset=\"#{url_base}#{instance[source][:generated_src]}\"#{media}>\n"
        }
        ...
        tag = %{
          ...
          #{source_tags}
          <!--[if IE 9]></video><![endif]-->
          <img srcset=\"#{url_base}#{instance['source_default'][:generated_src]}\" #{html_attr_string}>
          </picture>
        }
        ...
        if settings["lazysizes"]
          extra_attrs = %{ data-srcset="#{sizes.join(', ')}" class="js-lazyload lazyload" }
        else
